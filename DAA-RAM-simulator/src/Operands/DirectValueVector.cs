namespace DAA_RAM_simulator.src.Operands {
  public class DirectValueVector : IOperand {
    private int value_;
    private int index_;

    public DirectValueVector(string expression) {
    ...
    public int GetValue(IRegister[] registers, RegisterInt accumulator, Dictionary<string, int> tagMap) {
      return (value_ == 0) ? accumulator.GetData() : registers[value_ - 1].GetData(index_);
    }
  }
}